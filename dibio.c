...
int DiB_trainFromFilesToBuffer(char* &dictBuffer, unsigned maxDictSize,
                       const char** fileNamesTable, unsigned nbFiles, size_t chunkSize,
                       ZDICT_legacy_params_t *params, ZDICT_cover_params_t *coverParams,
                       int optimizeCover, double &executionTime)
{
    unsigned const displayLevel = params ? params->zParams.notificationLevel :
                        coverParams ? coverParams->zParams.notificationLevel :
                        0;   /* should never happen */
    dictBuffer = (char *)malloc(maxDictSize);
    fileStats const fs = DiB_fileStats(fileNamesTable, nbFiles, chunkSize, displayLevel);
    size_t* const sampleSizes = (size_t*)malloc(fs.nbSamples * sizeof(size_t));
    size_t const memMult = params ? MEMMULT : COVER_MEMMULT;
    size_t const maxMem =  DiB_findMaxMem(fs.totalSizeToLoad * memMult) / memMult;
...
//        EXM_THROW(14, "nb of samples too low");   /* we now clearly forbid this case */
    ...
    /* clean up */
_cleanup:
    clock_t end_dict = clock();
    executionTime += ((double) (end_dict - start_dict)) / CLOCKS_PER_SEC;
    free(srcBuffer);
    free(sampleSizes);
    return dictSize;
}
